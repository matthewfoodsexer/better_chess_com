(() => {
  ...
  // ══════════════════════════════════════════════════════════════
  // ██ Keyboard Moves (chess.com only)
  // ══════════════════════════════════════════════════════════════
  if (SITE === "chesscom") {
    const KB_FILE = { a:"a", s:"b", d:"c", f:"d", q:"e", w:"f", e:"g", r:"h" };
    const KB_RANK = { j:"1", k:"2", l:"3", ";":"4", u:"5", i:"6", o:"7", p:"8" };
    let kbBuf = [];
    let kbMoveEnabled = true;

    chrome.storage.sync.get("chessBetter", (res) => {
      const cfg = res.chessBetter || {};
      kbMoveEnabled = cfg.kbMoves !== false;
    });
    chrome.storage.onChanged.addListener((changes) => {
      if (!changes.chessBetter) return;
      kbMoveEnabled = (changes.chessBetter.newValue || {}).kbMoves !== false;
      if (!kbMoveEnabled) { kbBuf = []; updateKbHUD(); }
    });
    ...
    function isBoardFlipped() {
      const b = getBoard();
      if (!b) return false;
      return b.classList.contains("flipped") || b.getAttribute("flipped") != null;
    }
    ...
    function squareCenter(file, rank) {
      ...
      const fi = file.charCodeAt(0) - 97; // 0-7
      const ri = parseInt(rank) - 1;       // 0-7
      const flipped = isBoardFlipped();
      return {
        x: rect.left + (flipped ? 7 - fi : fi) * sz + sz / 2,
        y: rect.top + (flipped ? ri : 7 - ri) * sz + sz / 2,
      };
    }
    ...
    function simClick(x, y) {
      ...
      if (!el) return;
      const opts = { bubbles: true, clientX: x, clientY: y, pointerId: 1, pointerType: "mouse", button: 0 };
    ...
    function executeKbMove() {
      const [ff, fr, tf, tr] = kbBuf;
      const from = squareCenter(ff, fr);
      const to = squareCenter(tf, tr);
      if (!from || !to) return;
      simClick(from.x, from.y);
      setTimeout(() => simClick(to.x, to.y), 60);
    }

    // ── HUD indicator ───────────────────────────────────────
    function getKbHUD() {
      let hud = document.getElementById("cb-kb-hud");
      if (!hud) {
    ...
    function updateKbHUD() {
      const hud = getKbHUD();
      if (kbBuf.length === 0) { hud.style.display = "none"; return; }
      hud.style.display = "flex";
      const labels = [...kbBuf];
      while (labels.length < 4) labels.push("_");
      const from = labels[0] + labels[1];
      const to = labels[2] + labels[3];
      hud.innerHTML = `<span class="cb-kb-sq">${from}</span><span class="cb-kb-arrow">\u2192</span><span class="cb-kb-sq">${to}</span>`;

      // Position near the board
      const b = getBoard();
      if (b) {
        const rect = b.getBoundingClientRect();
        hud.style.left = rect.left + rect.width / 2 - 60 + "px";
        hud.style.top = rect.bottom + 8 + "px";
      }
    }

    // ── Key handler ─────────────────────────────────────────
    document.addEventListener("keydown", (ev) => {
      if (!kbMoveEnabled || isInputFocused()) return;

      if (ev.key === "Escape") { kbBuf = []; updateKbHUD(); return; }
      if (ev.key === "Backspace") { kbBuf.pop(); updateKbHUD(); ev.preventDefault(); return; }

      const key = ev.key.toLowerCase();
      const step = kbBuf.length;

      if (step === 0 || step === 2) {
        if (KB_FILE[key]) { kbBuf.push(KB_FILE[key]); updateKbHUD(); ev.preventDefault(); }
      } else if (step === 1 || step === 3) {
        if (KB_RANK[key]) {
          kbBuf.push(KB_RANK[key]);
          updateKbHUD();
          ev.preventDefault();
          if (kbBuf.length === 4) {
            executeKbMove();
            kbBuf = [];
            setTimeout(updateKbHUD, 150);
          }
        }
      }
    });

    console.log("[Chess Better] Keyboard moves enabled (L:file R:rank, 4-key)");
  }
})();
